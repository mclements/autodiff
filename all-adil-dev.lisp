...
;; Modified by Mark Clements <mark...@ki.se> 09 Nov 2017
...
(defstruct (df2 (:constructor df2 (f &optional (d 0) (d2 0)))) f d d2)

;;  print df structures with < , >
(defmethod print-object ((a df) stream)(format stream "<~a, ~a>" (df-f a)(df-d a)))
(defmethod print-object ((a df2) stream)(format stream "<~a, ~a, ~a>" (df2-f a)(df2-d a)(df2-d2 a)))
...
(defmethod ga::two-arg-+ ((a number)(b df)) 
  (df  (cl:+ a (df-f b))    (df-d b)))
(defmethod ga::two-arg-+ ((a sequence)(b sequence))
  (map 'list #'+ a b))
...
;; (sin (df (df 1.0d0 1) 1)) ; fails

;;; Multivariable functions (R^n -> R):
(defun make-gradient (function)
  #'(lambda (sequence)
      (let ((new-seq (copy-seq sequence)))
	(loop for index below (length sequence)
	   :do (setf (elt new-seq index) (df (elt sequence index) 1))
	   :collect (df-d (funcall function new-seq)) :into grad
	   :do (setf (elt new-seq index) (elt sequence index)) ; reset
	   :finally (return grad)))))
(defun gradient (function sequence)
  (funcall (make-gradient function) sequence))

#|
(defun dot (a b)
  (reduce #'+ (map 'list #'* a b)))
(let ((beta '(-6.0d0 3.0d0))
      (x '(2.0d0 3.0d0)))
  (flet ((f (x) (exp (dot beta x))))
    (list (gradient #'f x)
	  (list (* (f x) (elt beta 0))
		(* (f x) (elt beta 1))))))
|#
...
;; Apply this to partial likelihood?
